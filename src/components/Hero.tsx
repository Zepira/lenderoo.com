...
export default function Hero() {
  return (
    <section className="relative overflow-hidden pt-12 pb-20 sm:pt-16 sm:pb-24 lg:pt-20 lg:pb-32 bg-slate-50 dark:from-slate-950 dark:via-slate-900/40 dark:to-slate-950 transition-colors duration-300">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] border-[40px] border-teal-50/70 dark:border-teal-950/20 rounded-full pointer-events-none -z-0 opacity-75 dark:opacity-45"></div>
      <div className="absolute top-24 right-12 w-24 h-24 bg-teal-100 dark:bg-teal-950/40 rounded-3xl rotate-12 pointer-events-none -z-0 opacity-80"></div>
      <div className="absolute bottom-24 left-10 w-32 h-32 bg-amber-50 dark:bg-amber-950/20 rounded-full pointer-events-none -z-0 opacity-60"></div>

      {/* Decorative Blur Orbs */}
      <div className="absolute top-1/4 left-1/10 w-96 h-96 bg-teal-200 dark:bg-teal-950/15 rounded-full filter blur-3xl opacity-30 dark:opacity-10 pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/10 w-96 h-96 bg-amber-200 dark:bg-amber-950/10 rounded-full filter blur-3xl opacity-30 dark:opacity-10 pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col items-center lg:items-start text-center lg:text-left space-y-6 sm:space-y-8 relative z-10">
            ...
            {/* Friendly Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-teal-100 dark:bg-teal-950/60 border border-teal-200/50 dark:border-teal-900/60 text-[#00BFA6] dark:text-teal-300 text-xs font-bold uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5" />
              <span>On Android now, iOS in testing</span>
            </div>
            ...
            <div className="space-y-3">
              <h1 className="text-5xl sm:text-6xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-[1.1]">
                Share more, <br />
                <span className="text-[#00BFA6] dark:text-teal-400">buy less.</span>
              </h1>
              <p className="text-sm font-bold text-[#00BFA6] dark:text-teal-400 tracking-wide uppercase">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
              <a
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-[#00BFA6] hover:bg-[#0da48d] text-white px-7 py-3.5 rounded-2xl shadow-lg shadow-teal-200/50 dark:shadow-none hover:shadow-xl transition-all hover:-translate-y-0.5 cursor-pointer font-bold text-sm"
              >
            ...
            <div className="hidden sm:flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-250/40 dark:border-slate-800/50 max-w-lg mt-4 text-left shadow-xs">
              <div className="flex-shrink-0 bg-[#00BFA6]/10 p-1.5 rounded-xl border border-[#00BFA6]/20">
                <KangarooLogo size={42} variant="icon" />
              </div>
              <div>
                <span className="text-xs font-black text-teal-600 dark:text-teal-400 uppercase tracking-wide">Meet Lenderoo</span>
